...
class FormulaController extends Controller
{
    ...
    //   private static function checkZero(float $value, float $epsilon = $this->EPSILON): float
    // {
     ...
     * @return float
     */
    public function pmt(float $rate, int $periods, float $present_value, float $future_value = 0.0, bool $beginning = false): float
    {
        $when = $beginning ? 1 : 0;
        if ($rate == 0) {
    ...
    public static function rate(float $periods, float $payment, float $present_value, float $future_value, bool $beginning = false, float $initial_guess = 0.1): float
    {
        ...
        return $this->checkZero(NumericalAnalysis\RootFinding\NewtonsMethod::solve($func, [$initial_guess, $periods, $payment, $present_value, $future_value, $when], 0, $this->EPSILON, 0));
    }
    /**
    ...
    public static function irr(array $values, float $initial_guess = 0.1): float
    {
        ...
        return $this->checkZero(NumericalAnalysis\RootFinding\NewtonsMethod::solve($func, [$initial_guess, $values], 0, $this->EPSILON, 0));
    }
    /**
    ...
    public static function mirr(array $values, float $finance_rate, float $reinvestment_rate): float
    {
        ...
        $pv_inflows = $this->npv($reinvestment_rate, $inflows);
        $fv_inflows = $this->fv($reinvestment_rate, $root, 0, -$pv_inflows);
        $pv_outflows  = $this->npv($finance_rate, $outflows);
        return $this->checkZero(pow($fv_inflows / -$pv_outflows, 1 / $root) - 1);
    }
    /**
    ...
    //         return \NAN;
    //     }
    //     $pv_inflows  = $this->npv($rate, $inflows);
    //     $pv_outflows = $this->npv($rate, $outflows);
