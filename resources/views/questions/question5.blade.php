...
$auth = \Auth::user();
if(!empty($auth)){
    $ques = \App\question_survey::where('user_id',\Auth::user()->id)->where('question_id','5')->get();
    if(count($ques)){
    $ques5 = $ques;
	}
}
...
<div class="ques_6" id="ques_6" <?php if(isset($ques5)){ }else{ echo'style="display:none"'; } ?>>
	<div class="q6 chat-container d-f f-d-c a-i-f-s" id="formQuestionFivePointOneDIV">
		...
		<div class="message no-background d-i-f a-i-c">
			...
			<form method="post" class="form-inline average-savings" id="formQuestionFivePointOne">
				<ul class="question5-ul my_question ">
					<li class="">
						...
						<input type="hidden" name="stable_status_fee" value="">
			...
			<div class="form-group top-m">
				<form method="post" id="formQuestionFive" class="form-inline average-savings">
					<div class="form-group">
						<input type="text" id="average-savings" class="form-control" data-att="Our economic situation is not stable" name="stable_status_fee" value="<?php if(isset($ques5)){ echo $ques5[0]->meta_value; }else{ echo""; } ?>" placeholder="חסכון חודשי מעל 1,000 שקלים" onkeyup="this.value=addCommas(this.value);"/>
